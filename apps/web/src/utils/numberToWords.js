...
export function numberToWords(amount) {
    ...
    const parts = [];
    if (billions)
        parts.push(`${chunkUnderThousand(billions)} billion`);
    if (millions)
        parts.push(`${chunkUnderThousand(millions)} million`);
    if (thousands)
        parts.push(`${chunkUnderThousand(thousands)} thousand`);
    if (remainder)
        parts.push(chunkUnderThousand(remainder));
    return parts.join(" ");
}
// Capitalises the first letter — the SPA uses sentence case for amounts.
export function numberToWordsCapitalized(amount) {
    const w = numberToWords(amount);
    return w.charAt(0).toUpperCase() + w.slice(1);
}
// "AED 26,320 (Dirhams Twenty-six thousand three hundred twenty only)"
export function aedAmountInWords(amount) {
