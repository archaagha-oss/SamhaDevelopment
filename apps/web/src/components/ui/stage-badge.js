...
export function StageBadge({ stage, kind = "lead", label, className, ...props }) {
    const map = kind === "deal" ? DEAL_STAGE_TONE : LEAD_STAGE_TONE;
    const tone = map[stage] ?? "neutral";
    const display = label ?? stage.replace(/_/g, " ");
    return (_jsx(Badge, { variant: "outline", className: cn("border-transparent", TONE_CLASS[tone], className), ...props, children: display }));
}
