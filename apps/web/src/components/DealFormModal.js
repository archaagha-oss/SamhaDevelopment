...
export default function DealFormModal({ onClose, onCreated, defaultLeadId }) {
    ...
    return (_jsx(Dialog, { open: true, onOpenChange: (o) => { if (!o)
            handleClose(); }, children: _jsxs(DialogContent, { className: "max-w-2xl max-h-[92vh] flex flex-col p-0 gap-0", children: [_jsxs("div", { className: "px-6 pt-5 pb-4 border-b flex-shrink-0", children: [_jsx("div", { className: "flex items-center justify-between mb-4", children: _jsx("h2", { className: "font-bold text-foreground text-lg", children: "New Deal" }) }), _jsx("div", { className: "flex items-center gap-0", children: STEPS.map((label, i) => {
                                const done = i < step;
                                const current = i === step;
                                return (_jsxs("div", { className: "flex items-center flex-1 last:flex-none", children: [_jsxs("button", { onClick: () => done && setStep(i), disabled: !done, className: "flex flex-col items-center gap-1 group disabled:cursor-default", children: [_jsx("div", { className: `w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all border-2 ${done ? "bg-primary border-primary text-primary-foreground" :
                                                        current ? "bg-background border-primary text-primary" :
                                                            "bg-muted border-border text-muted-foreground"}`, children: done ? "✓" : i + 1 }), _jsx("span", { className: `text-[10px] font-semibold whitespace-nowrap ${current ? "text-primary" : done ? "text-primary/70" : "text-muted-foreground"}`, children: label })] }), i < STEPS.length - 1 && (_jsx("div", { className: `flex-1 h-0.5 mx-1 mb-4 transition-colors ${i < step ? "bg-primary/70" : "bg-border"}` }))] }, i));
