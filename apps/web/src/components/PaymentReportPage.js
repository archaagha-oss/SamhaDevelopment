...
export default function PaymentReportPage() {
    ...
    const [upcomingView, setUpcomingView] = useState("7");
    ...
    return (_jsxs("div", { className: "p-6 space-y-5", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { children: [_jsx("h1", { className: "text-lg font-bold text-slate-900", children: "Payment Collections" }), _jsx("p", { className: "text-slate-400 text-xs mt-0.5", children: "Track and manage all payment milestones" })] }), _jsx("button", { onClick: load, className: "text-xs text-slate-500 hover:text-slate-800 border border-slate-200 rounded-lg px-3 py-1.5 transition-colors", children: "Refresh" })] }), activeStatus === "OVERDUE" && collections && (_jsx("div", { className: "grid grid-cols-2 sm:grid-cols-4 gap-3", children: collections.aging.map(({ range, count, amount }) => {
                    ...
                    return (_jsxs("div", { className: `rounded-xl border p-4 ${colors[range] || "bg-slate-50 border-slate-200 text-slate-600"}`, children: [_jsxs("p", { className: "text-xs font-bold uppercase tracking-wide mb-1", children: [range, " days overdue"] }), _jsx("p", { className: "text-2xl font-bold", children: count }), _jsxs("p", { className: "text-xs mt-0.5 opacity-80", children: ["AED ", amount.toLocaleString()] })] }, range));
                }) })), _jsx("div", { className: "grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-9 gap-2", children: STATUS_ORDER.map((status) => {
                ...
                }) }), isUpcoming && (_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: "text-xs text-slate-500 font-medium", children: "Show:" }), ["7", "30"].map((d) => (_jsxs("button", { onClick: () => setUpcomingView(d), className: `px-3 py-1 text-xs font-semibold rounded-lg border transition-colors ${upcomingView === d ? "bg-blue-600 text-white border-blue-600" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`, children: ["Next ", d, " days"] }, d))), collections && (_jsx("span", { className: "ml-2 text-xs text-slate-400", children: upcomingView === "7"
